...
<body>
    ...
    <div class="container-fluid">
        <!-- Aquí se inserta la información de los productos -->
        <?php
...
try {
    ...
    if ($resultado) {
        // Mostrar los productos directamente en la página HTML
...
?>
    ...
    // Realizar solicitud AJAX al servidor para agregar a favoritos
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            // Cambiar la apariencia del botón según la respuesta del servidor
            if (xhr.responseText.includes("ya está en favoritos")) {
                button.classList.remove("btn-danger");
                button.classList.add("btn-outline-danger");
                button.disabled = true; // Desactivar el botón si ya está en favoritos
            } else {
                button.classList.remove("btn-outline-danger");
                button.classList.add("btn-danger");
                button.disabled = false; // Habilitar el botón si se agregó a favoritos
            }
        }
    };
    xhr.send();
